...
export async function POST(req: Request) {
  try {
    const body = await req.json(); // objeto curso: { id, name, parcialCapacity, capacity }
    const jwt = await getBackendJWT();
    const base = process.env.NEXT_PUBLIC_URL?.replace(/\/+$/,"") || "http://localhost:8080";

    const r = await fetch(`${base}/usr/registerCourse`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ message: e?.message || "Error inscribiendo curso" }, { status: 500 });
  }
}
